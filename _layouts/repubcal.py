...
def annee_de_la_revolution(date):
    ...
    lasteq = previous_autumnal_equinox(date)
    nexteq = next_autumn_equinox(date)
...
class RDate(dt.date):
    ...
    def revol_strftime(self, fmt):
        ...
        newformat = []
        def push(val): return newformat.append(str(val))
        i, n = 0, len(fmt)
        while i < n:
            char = fmt[i]
            i += 1
                        ...
                        if i < n:
                            ...
                            if char == "A":
                                ...
                                if rdate['mois'] >= len(REV_MONTH_NAMES):
                                    ...
                                    resource = FETES[rdate['mois']
                                                     ][rdate['jour']][1]
                                    push("{}{}".format(WIKI_BASE_URL,
                                                       resource.replace(" ", "_")))
                            elif char == "u":
                                # unicode of the day.
                                if rdate['mois'] >= len(REV_MONTH_NAMES):
